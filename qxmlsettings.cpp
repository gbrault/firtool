...
static bool nodeToVariant(const QDomNode &aNode,QVariant &aValue) {
    ...
    else if(vType == QString::fromLatin1("ulong")) {
        aValue = vValue.toULongLong(&vRetval);
    }
    else if(vType == QString::fromLatin1("long")) {
        aValue = vValue.toLongLong(&vRetval);
    }
    else if(vType == QString::fromLatin1("integer")) {
        aValue = vValue.toInt(&vRetval);
    }
    else if(vType == QString::fromLatin1("uint")) {
        aValue = vValue.toUInt(&vRetval);
    }
...
static bool variantToNode(const QVariant &aValue, QDomElement &aElement) {
    ...
    switch (aValue.type()) {
        ...
        case QVariant::ULongLong: {
            vType  = QString::fromLatin1("ulong");
            vValue = aValue.toString();
            break;
        }
        case QVariant::LongLong: {
            vType  = QString::fromLatin1("long");
            vValue = aValue.toString();
            break;
        }
        case QVariant::Int: {
