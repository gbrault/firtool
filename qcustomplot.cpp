...
void QCPAxis::setupTickVectors()
{
  ...
  if (mAutoTickLabels)
  {
    ...
    if (mTickLabelType == ltBell)
    {
      for (int i=0; i<vecsize; ++i)
        mTickVectorLabels[i] = mParentPlot->locale().toString(20*log10(mTickVector.at(i)), mNumberFormatChar, mNumberPrecision);
    } else
    if (mTickLabelType == ltDateTime) {
      for (int i=0; i<vecsize; ++i)
      {
...
void QCPAxis::drawTickLabel(QCPPainter *painter, double position, int distanceToAxis, const QString &text, QSize *tickLabelsSize)
{
  ...
  */
  ...
  {
    // draw base:
    painter->drawText(0, 0, 0, 0, Qt::TextDontClip, text);
    // draw exponent:
